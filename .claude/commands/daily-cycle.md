...
## Step 1. 데이터 페치 & D+1 AS 추천 (Bash, 순차)

```bash
python shared/scripts/fetch_pnl_data.py        # 어제 실적
python shared/scripts/fetch_market_data.py     # 내일 시황 (yes-energy/{내일}.csv 생성)
python shared/scripts/recommend_as_position.py # D+1 AS playbook 추천 (위 CSV 소비 → as-playbook/{내일}.json)
```

각 명령의 stdout 마지막 5줄만 보여주고 다음 단계로.
`recommend_as_position.py` 가 실패해도 (YE bidclose 피드 저하 등) 사이클은 멈추지 않고 계속 — `bess-optimizer` 가 playbook prior 없이 자체 판단한다.

## Step 2. P&L 정리

Task 도구로 `pnl-manager` 호출:
- Input: Step 1의 `fetch_pnl_data.py` 출력 (`shared/data/raw/` — Tenaska + Smartbidder benchmark)
- Expected output: `reports/daily/pnl/{어제}.md` + 정제 parquet (`shared/data/pnl/gks/{hourly,daily}/`, `shared/data/benchmarks/smartbidder/{hourly,daily}/`)
...
- Task → `bess-optimizer`: GKS DA/RT Energy + AS revenue stack. **Step 1의 `shared/data/forecasts/as-playbook/{내일}.json` (fine playbook 시간별 AS 추천)을 AS 상품/venue 배분 prior로 활용** (in-sample 학습이므로 절대 룰이 아닌 prior — 가격 view와 충돌 시 bess-optimizer 판단 우선)
...
- `recommend_as_position.py` 실패 (YE bidclose 피드 저하 등) → 사이클 계속, bess-optimizer는 playbook prior 없이 진행
